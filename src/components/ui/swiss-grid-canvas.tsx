			...
			for (const section of sections) {
				...
				if (verticalProgress >= trigger) {
					...
					if (lineProgress > 0) {
						...
						// Apple-Level Kinetic Extrusion:
						// No more globalAlpha ghosts. Bars grow from center based on head position.
						const drawIntersect = (ix: number) => {
							...
							// How far past the intersection is the drawing head?
							const distPast = currentX - railX;

							if (distPast > 0) {
								// cornerFactor goes 0 -> 1 as the head travels one full corner length past the joint
								const cornerFactor = Math.min(distPast / CORNER_DASH_SIZE, 1);
								// Overshoot/vibration only at the very end of the line
								const impact =
									lineProgress > 0.98
								...
								// Ink density reinforcement (Grows with joint maturity)
