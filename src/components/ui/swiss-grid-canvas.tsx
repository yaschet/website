    ...
    (progress: number) => {
      ...
      if (progress > 0) {
        ...
        drawVerticalRail(
          ...
          horizontalYs,
          dashColor,
          dashColor // Use regular dash color for scaffold corners (faint)
        );
        drawVerticalRail(
          ctx,
          containerRight,
        ...
        for (const s of sections) {
          drawHorizontalLine(
            ...
            containerRight,
            dashColor,
            dashColor // Scaffold corners are faint and same color
          );
        }
        ctx.globalAlpha = 1;
      }
      ...
      for (const section of sections) {
        ...
        if (progress >= trigger) {
          ...
          if (lineProgress > 0) {
            ...
            // PASS 4: Apple-Level "Physical Stamp" Corners (Pure geometry)
            const drawPhysicalStamp = (ix: number) => {
              ...
              if (distPast > 0) {
                ...
                // Sub-pixel sharp scale: 0 -> 1.0 (No vibration or over-scaling)
                const stampScale =
                  rawStampProgress < 0.6
                    ? (rawStampProgress / 0.6) * 1.05
                    : 1.05 - ((rawStampProgress - 0.6) / 0.4) * 0.05;
                ...
                // The "Stamped" Mark (V + H) - Just like the original
                ctx.fillRect(
                  -halfCornerThickness,
                  -halfBar,
                  CORNER_THICKNESS,
...
function drawVerticalRail(
  ...
  while (currentY < height) {
    // Skip corner reinforcement zones ALWAYS (to leave room for bars)
