...
>(({ size: propSize, ...props }, ref) => {
	...
	return <DropdownMenuPrimitive.Trigger ref={ref} {...props} data-size={size} />;
});
...
const dropdownMenuItemVariants = cva(
	"relative flex size-auto flex-1 cursor-pointer select-none flex-row items-center justify-start gap-2 whitespace-nowrap outline-none transition-transform duration-75 ease-in-out focus:bg-surface-200 focus:text-surface-800 active:scale-[0.95] data-[disabled]:pointer-events-none data-[disabled]:opacity-50 dark:focus:bg-surface-900 dark:focus:text-surface-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
	{
		variants: {
			size: {
				// Border radius follows nesting: items are 1/2 of menu radius
				xs: "rounded-lg px-2.5 py-1 font-medium text-xs [&_svg]:size-3.5",
				sm: "rounded-xl px-3 py-1.5 font-medium text-xs [&_svg]:size-4",
				md: "rounded-xl px-4 py-2 font-medium text-sm [&_svg]:size-5",
				lg: "rounded-2xl px-5 py-2.5 font-medium text-sm [&_svg]:size-5",
				xl: "rounded-[1.25rem] px-6 py-3 font-medium text-lg [&_svg]:size-6",
			},
			inset: {
				true: "pl-8",
				false: "",
...
const dropdownMenuContentVariants = cva(
	"z-50 mt-2.5 flex min-w-[8rem] flex-col overflow-hidden border border-border bg-surface-2 text-foreground shadow-sm",
	{
		variants: {
			size: {
				xs: "rounded-xl p-1.5",
...
>(({ allowOverflow = false, className, sideOffset = 4, size: propSize, ...props }, ref) => {
...
>(({ children, className, ...props }, ref) => {
	...
	return (
		...
		>
			<DropdownMenuPrimitive.ItemIndicator
				className={cn(
					"absolute top-0 flex h-full items-center justify-center text-primary",
					size === "xs" && "left-2",
					size === "sm" && "left-2",
...
>(({ className, inset, ...props }, ref) => {
	...
	return (
		<DropdownMenuPrimitive.Label
			ref={ref}
			className={cn(
				"font-semibold dark:text-surface-200",
				size === "xs"
...
>(({ className, ...props }, ref) => {
	...
	return (
		<DropdownMenuPrimitive.Separator
			ref={ref}
			className={cn(
				"h-px bg-border dark:bg-surface-800",
				size === "xs"
					? "-mx-1.5 my-1.5"
					: size === "sm"
						? "-mx-2 my-2"
...
function DropdownMenuShortcut({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) {
	return (
		<span
			className={cn(
				"ml-auto text-xs tracking-widest opacity-60 dark:text-surface-400 dark:opacity-80",
				className,
			)}
			{...props}
		/>
