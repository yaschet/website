...
const progressVariants = cva(
    ['relative h-2 w-full overflow-hidden rounded-full bg-primary/20', 'size-full flex-1 bg-primary transition-all'],
    {
        defaultVariants: {
            background: 'primary',
            color: 'primary',
    ...
    ({ className, color, size, value, ...props }, ref) => (
        ...
        >
            <ProgressPrimitive.Indicator
                className={cn('size-full flex-1 transition-all', progressVariants({ background: 'none', color }))}
